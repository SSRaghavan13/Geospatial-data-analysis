...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = 
  {
    ...
    val answerDf = spark.sql("select rectangle,count(point) as freq from joinResult group by rectangle order by rectangle")

    return answerDf // YOU NEED TO CHANGE THIS PART
  }
