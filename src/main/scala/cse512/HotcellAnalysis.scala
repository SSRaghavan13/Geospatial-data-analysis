...
def runHotcellAnalysis(spark: SparkSession, pointPath: String): DataFrame =
{
  ...
  val countsDF = spark.sql(freqSQL.format(minX, maxX, minY, maxY, minZ, maxZ))
 
  countsDF.createOrReplaceTempView("counts")

  // Calculate sigma Xi and sigma X square i
  val freqDF = spark.sql("select sum(frequency), sum(frequency*frequency) from counts")

  // Value of X bar
  val X = freqDF.first().getLong(0).toDouble/nCells

  // Value of S
  val S = Math.sqrt(freqDF.first().getLong(1).toDouble/nCells - X*X)

  spark.udf.register("getNeighbors", (maxX: Double, minX: Double, maxY: Double, minY: Double, maxZ: Int, minZ: Int, X: Double, Y: Double, Z: Int)=>((
    HotcellUtils.getNeighbors(maxX, minX, maxY, minY, maxZ, minZ, X, Y, Z)
    )))

  val wxSQL = "select getNeighbors( %f, %f, %f, %f, %d, %d, a.x, a.y, a.z) as Wij, sum(b.frequency) as Xj, a.x as X, a.y as Y, a.z as Z from counts a, counts b where" +
  ...
  val wxDF = spark.sql(wxSQL.format(maxX, minX, maxY, minY, maxZ, minZ))

  wxDF.createOrReplaceTempView("wxValues")
  ...
  val zScoreSQL = "select x, y, z, calculateZ(Wij, Xj, %f, %f, %f) as ZScore from wxValues"
